...
//      Adds a space in between each printed char
...
//  Uses the above function but takes in a string as the parameter
struct CircularLinkedList * createListFromString(std::string in);


int main(){
    std::string str = "I Love Computer Science";
    struct CircularLinkedList * list = createListFromString(str);
    printList(list);

    addToList(list, '!');
    struct node * removed = popAtIndex(2, list);
    printf("Removed value = %c\n", removed->val);
    //if you pop a value be careful because its no longer tracked in deleteList
    //function
    free(removed);
 ...
 */
struct CircularLinkedList * createCircularList(char value){
    struct CircularLinkedList * list = new struct CircularLinkedList;
...
struct CircularLinkedList * createListFromString(std::string in){
    ...
    struct CircularLinkedList * list = createListFromConstChar(in.c_str(), length);
